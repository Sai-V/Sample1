...
def main():
    ...
    reward=Price(
        amount=X.XX                                               # reward for HIT completion
        )
    ...
    create_hit_result = connection.create_hit(
        ...
        max_assignments=A,                                        # number of assignments
        lifetime=datetime.timedelta(days=B),                      # time HIT is available
        duration=datetime.timedelta(minutes=C),                   # time worker has to complete HIT once accepted
        approval_delay=datetime.timedelta(days=D),                # time until HIT is automatically approved
        question=question,
        reward=reward,
